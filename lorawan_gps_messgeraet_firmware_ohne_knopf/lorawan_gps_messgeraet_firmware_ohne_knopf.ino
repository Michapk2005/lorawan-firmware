...
// ANGEPASST: 45s Intervall - CS sendet jeden Zyklus, TTN jeden 2. Zyklus (= alle 90s)
...
// A ping is only saved to the EEPROM, if there is no other ping in a radius of 10 meters saved already.
...
// Gets new GPS-Coordinates and sends it via LoRaWAN, if the previous ping is more than 9m away.
static bool prepareTxFrame( uint8_t port ) 
{
  ...
  // From this point on a ping will be sent via LoRaWAN:
  ...
  deviceStatusBatteryLevel = map(getBatteryVoltage(), 3300, 4200, 1, 254);
  uint16_t currentBattery = getBatteryVoltage();
...
// dumpEEPROM writes the saved pings out for the Funkloch-Upload
void dumpEEPROM() {
